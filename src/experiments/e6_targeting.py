...
from src.evaluation.uplift import compute_qini_coefficient
...
def run_e6(config_path: str):
    """
    E6: Business Targeting (SHOULD).
    Uses Criteo dataset (or synthetic fallback) to evaluate the estimators 
    via Qini coefficient and Top-K targeting since we cannot compute PEHE.
    """
    logger.info("=== Starting E6: Business Targeting (Criteo) ===")
    ...
    for name, learner in [('S-Learner', s_learner), ('T-Learner', t_learner), ('X-Learner', x_learner)]:
        ...
        # Calculate Qini Coefficient (Area under the curve - Area under random baseline)
        qini_coeff = compute_qini_coefficient(np.asarray(Y_test), np.asarray(T_test), tau_hat)
        
        logger.info(f"\n--- {name} | Mean Pred Effect: {mpe:.4f} | Qini Coefficient (Area): {qini_coeff:.4f} ---")
        
        # Top-K targeting stats (Cumulative incremental gain at specific fractions)
        stats = compute_topk_fraction_stats(
            np.asarray(Y_test), np.asarray(T_test), tau_hat, k_fractions
        )
        logger.info(f"Top-K Targeting (Incremental Gain at specific fractions):\n{stats.to_string(index=False)}")
